...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      if (mounted && data.session) navigate("/chat");
    });
  ...
  const formatError = (err) => err?.message || String(err) || "Something went wrong.";
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (error) setNotice({ type: "error", message: formatError(error) });
    } catch (err) { setNotice({ type: "error", message: formatError(err) }); }
  ...
  const handleEmailAuth = async (e) => {
    ...
    try {
      if (authMode === "signin") {
        ...
        if (error) { setNotice({ type: "error", message: formatError(error) }); return; }
        if (data?.session) navigate("/chat");
      } else {
        const { data, error } = await supabase.auth.signUp({ email, password });
        if (error) { setNotice({ type: "error", message: formatError(error) }); return; }
        if (data?.session) { navigate("/chat"); return; }
        setNotice({ type: "success", message: "Check your email to confirm your account." });
      }
    } catch (err) { setNotice({ type: "error", message: formatError(err) }); }
    finally { setLoading(false); }
  };
  ...
  return (
    <div className="min-h-[100dvh] bg-white flex flex-col items-center justify-center px-4 py-10">
      {/* Logo */}
      <div className="flex flex-col items-center mb-8">
        <img src={appIcon} alt="App icon" className="h-14 w-14 rounded-2xl shadow-md mb-3" />
        <h1 className="text-2xl font-bold text-gray-900">My Chat App</h1>
      </div>
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="flex rounded-xl bg-gray-100 p-1 mb-6">
          {["signin", "signup"].map((mode) => (
            <button key={mode} type="button"
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition ${authMode === mode ? "bg-white shadow text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
        ...
        <div className="flex gap-2 mb-5">
          {["email", "phone"].map((m) => (
            <button key={m} type="button"
              ...
              className={`flex-1 py-2 rounded-xl border text-sm font-semibold transition ${method === m ? "border-blue-500 bg-blue-50 text-blue-600" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
              {m === "email" ? "Email" : "Phone"}
            </button>
          ))}
        </div>
        ...
        {/* Notice */}
        {notice && (
          <div className={`mb-4 rounded-xl px-4 py-3 text-sm ${notice.type === "error" ? "bg-red-50 text-red-600 border border-red-200" : "bg-green-50 text-green-700 border border-green-200"}`}>
            {notice.message}
          </div>
        )}
        ...
        {method === "email" ? (
          <form onSubmit={handleEmailAuth} className="space-y-4">
            <div>
              ...
              <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 focus-within:border-blue-400 focus-within:bg-white transition">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 focus-within:border-blue-400 focus-within:bg-white transition">
                ...
                <button type="button" onClick={() => setShowPassword((v) => !v)} className="text-gray-400 hover:text-gray-600 transition">
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            </div>
            <button type="submit" disabled={loading}
              className="w-full rounded-xl bg-blue-500 hover:bg-blue-600 text-white py-3 text-sm font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed shadow-sm">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 focus-within:border-blue-400 focus-within:bg-white transition">
                  ...
                  <button type="button" onClick={() => { setOtpSent(false); setOtp(""); setNotice(null); }}
                    className="text-xs text-blue-500 hover:text-blue-700 font-semibold transition">Change</button>
                </div>
              </div>
            )}
            <button type="submit" disabled={loading}
        ...
        <div className="space-y-3">
          <button type="button" onClick={handleGoogle} disabled={loading}
            className="w-full flex items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 py-3 text-sm font-semibold text-gray-700 transition shadow-sm disabled:opacity-60">
